...
int AllShocksCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, LIGHTNING);
    return -1;
}

int AllMegasCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, MEGA_MUSHROOM);
    return -1;
}

int AllFeathersCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, BLOOPER);
    return -1;
}

int AllPOWsCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, POW_BLOCK);
    return -1;
}

int AllGoldensCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, MUSHROOM);
    return -1;
}

int AllBulletsCanLand(Kart::Collision *kartCollision){
    if (System::sInstance->IsContext(PULSAR_MEGATC)) return UseItem(kartCollision, BULLET_BILL);
    return -1;
}
